...
function _merge_outer(
    ::Type{IndexType}, ta1::TimeArray{T,N,D}, ta2::TimeArray{T,M,D}, padvalue, meta
) where {IndexType,T,N,M,D}
    timestamps, new_idx1, new_idx2 = sorted_unique_merge(
        IndexType, timestamp(ta1), timestamp(ta2)
    )
    vals = fill(
        convert(T, padvalue),
        (length(timestamps), length(colnames(ta1)) + length(colnames(ta2))),
    )
    insertbyidx!(vals, values(ta1), new_idx1)
    insertbyidx!(vals, values(ta2), new_idx2, size(values(ta1), 2))
    return TimeArray(timestamps, vals, [colnames(ta1); colnames(ta2)], meta; unchecked=true)
end
...
function merge(
    ...
    meta = if _meta(ta1) == _meta(ta2) && meta ≡ nothing
        _meta(ta1)
    elseif typeof(_meta(ta1)) <: AbstractString &&
        typeof(_meta(ta2)) <: AbstractString &&
        meta ≡ nothing
    ...
    elseif method == :right
        ...
        ta = TimeArray(
            timestamp(ta), vals, [_colnames(ta1); _colnames(ta2)], meta; unchecked=true
        )
    ...
    elseif method == :outer
        ...
        throw(ArgumentError("merge method must be one of :inner, :left, :right, :outer"))
    end
...
function merge(
    ...
    return rename!(merge(merge(x, y; kw...), z, a...; kw...), colnames)
end
...
for (F, T, P) in (
    ...
    @eval let
        ...
        function collapse(ta::TimeArray, ::typeof($F), f::Function, g::Function=f; kw...)
            return collapse(ta, wrapper, f, g; kw...)
        end
    end
end

function collapse(
    ta::TimeArray, period::Function, timestamp::Function, value::Function=timestamp
)
    ...
    ts′ = [timestamp(@view(ts[i])) for i in idx]
    val′ = if n == 1
        [value(@view(val[i])) for i in idx]
    else
        [value(@view(val[i, k])) for i in idx, k in 1:n]
    end

    return TimeArray(ts′, val′, colnames(ta), meta(ta))
end
