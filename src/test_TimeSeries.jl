...
#import Calendar
using Calendar # this one is needed

export imfred

function imfred()
  parse("yyyy-MM-dd", "2012-12-12")
end

end #of module 

###################### the real mcCoy below ################
...
#    #ts = [TimeStamp(parse("yyyy-MM-dd", timestamps[1]), float(vals[1]))]
#    ts = [TimeStamp(parse("yyyy-MM-dd", "2012-12-12"), float(vals[1]))]
#    for i in 2:length(timestamps)
#      obj = TimeStamp(parse("yyyy-MM-dd", timestamps[i]), float(vals[i]))
#      ts  = push!(ts, obj)
#    end
#    ts
#end
