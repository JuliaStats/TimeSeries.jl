...
@testset "combine" begin
    @testset "collapse operations" begin
        @testset "collapse squishes correctly" begin
            ...
            @test values(collapse(ohlc, week, first, last))[2, :] ==
                [100.0, 102.25, 99.38, 100.44]
            @test timestamp(collapse(ohlc, week, first, last))[2] == Date(2000, 1, 10)

            @test values(collapse(ohlc, month, first))[2, :] ==
                [104.0, 105.0, 100.0, 100.25]
            ...
            let  # week
                ts = [
                    Date(2018, 1, 2), Date(2018, 1, 3), Date(2019, 1, 5), Date(2019, 2, 6)
                ]
                ta = TimeArray(ts, 1:length(ts))
            ...
            let  # month
                ts = [
                    Date(2018, 1, 2), Date(2018, 1, 3), Date(2019, 1, 10), Date(2019, 2, 5)
                ]
                ta = TimeArray(ts, 1:length(ts))
        ...
        @testset "Period supports" begin
            ts = [Date(2018, 1, 2), Date(2018, 1, 3), Date(2019, 1, 5), Date(2019, 2, 6)]
            ta = TimeArray(ts, 1:length(ts))
    ...
    @testset "merge works correctly" begin
        ...
        @testset "takes colnames kwarg correctly" begin
            @test colnames(merge(cl, ohlc[:High, :Low]; colnames=[:a, :b, :c])) ==
                [:a, :b, :c]
            ...
            for mode in [:inner, :left, :right, :outer]
                @test colnames(
                    merge(cl, ohlc[:High, :Low]; method=mode, colnames=[:a, :b, :c])
                ) == [:a, :b, :c]
    ...
    @testset "vcat works correctly" begin
        ...
        @testset "concatenates time series correctly in 2D" begin
            a = TimeArray(
                [Date(2015, 09, 01), Date(2015, 10, 01), Date(2015, 11, 01)],
                [[15 16]; [17 18]; [19 20]],
                [:Number1, :Number2],
            )
        ...
        @testset "rejects when metas do not match" begin
            a = TimeArray(
                [Date(2015, 10, 01), Date(2015, 11, 01)], [15, 16], [:A], :FirstMeta
            )
        ...
        @testset "duplicated timestamps order" begin
            a = TimeArray(
                [Date(2015, 10, 1), Date(2015, 10, 2), Date(2015, 11, 1)], [15, 16, 17]
            )
    ...
    @testset "map works correctly" begin
        ...
        @testset "works on both time stamps and 2D values" begin
            ...
            b = map(
                (timestamp, values) ->
                    (timestamp + Dates.Year(1), [values[1] + 2, values[2] - 1]),
                a,
            )
        ...
        @testset "works with order of elements that varies after modifications" begin
            ...
            b = map(
                (timestamp, values) -> (
                    timestamp + Dates.Year((timestamp >= Date(2015, 11, 01)) ? -1 : 1),
                    values,
                ),
                a,
            )
