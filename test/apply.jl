...
@testset "apply" begin
    @testset "time series methods" begin
        @testset "lag takes previous day and timestamps it to next day" begin
            ta = lag(cl)
            @test values(ta)[1] ≈ 111.94 atol = 0.01
        ...
        @testset "diff calculates 1st-order differences for multi-column ts" begin
            ...
            @test values(diff(ohlc; padding=false))[1, :] ==
                values(ohlc)[2, :] .- values(ohlc)[1, :]
            @test values(diff(ohlc; padding=true))[2, :] == values(diff(ohlc))[1, :]
            @test values(diff(ohlc; padding=true))[2, :] ==
                values(ohlc)[2, :] .- values(ohlc)[1, :]
            @test all(x -> isnan(x), values(diff(ohlc; padding=true))[1, :])
        end

        @testset "diff calculates 2nd-order differences" begin
            @test timestamp(diff(op; differences=2)) ==
                timestamp(diff(op; padding=false, differences=2))
            @test timestamp(diff(diff(op))) ==
                timestamp(diff(op; padding=false, differences=2))

            @test values(diff(op; differences=2)) ==
                values(diff(op; padding=false, differences=2))
            @test values(diff(diff(op))) == values(diff(op; padding=false, differences=2))

            @test values(diff(op; padding=true, differences=2))[3] ==
                values(diff(op; differences=2))[1]
        ...
        @testset "diff calculates 2nd-order differences for multi-column ts" begin
            @test timestamp(diff(ohlc; differences=2)) ==
                timestamp(diff(ohlc; padding=false, differences=2))
            @test timestamp(diff(diff(ohlc))) ==
                timestamp(diff(ohlc; padding=false, differences=2))

            @test values(diff(ohlc; differences=2)) ==
                values(diff(ohlc; padding=false, differences=2))
            @test values(diff(diff(ohlc))) ==
                values(diff(ohlc; padding=false, differences=2))
            @test values(diff(ohlc; padding=true, differences=2))[3, :] ==
                values(diff(ohlc; differences=2))[1, :]
        ...
        @testset "diff calculates 3rd-order differences" begin
            @test timestamp(diff(op; differences=3)) ==
                timestamp(diff(op; padding=false, differences=3))
            @test values(diff(op; differences=3)) ==
                values(diff(op; padding=false, differences=3))
            @test timestamp(diff(diff(diff(op)))) ==
                timestamp(diff(op; padding=false, differences=3))
            @test values(diff(diff(diff(op)))) ==
                values(diff(op; padding=false, differences=3))
            @test values(diff(op; padding=true, differences=3))[4] ==
                values(diff(op; differences=3))[1]
        ...
        @testset "diff calculates 3rd-order differences for multi-column ts" begin
            @test timestamp(diff(ohlc; differences=3)) ==
                timestamp(diff(ohlc; padding=false, differences=3))
            @test values(diff(ohlc; differences=3)) ==
                values(diff(ohlc; padding=false, differences=3))
            @test timestamp(diff(diff(diff(ohlc)))) ==
                timestamp(diff(ohlc; padding=false, differences=3))
            @test values(diff(diff(diff(ohlc)))) ==
                values(diff(ohlc; padding=false, differences=3))
            @test values(diff(ohlc; padding=true, differences=3))[4, :] ==
                values(diff(ohlc; differences=3))[1, :]
        ...
        @testset "simple return value" begin
            ...
            @test values(percentchange(cl))[1] ≈ (102.5 - 111.94) / 111.94 atol = 0.01
            @test values(percentchange(ohlc))[1, :] ≈
                (values(ohlc)[2, :] - values(ohlc)[1, :]) ./ values(ohlc)[1, :]
            @test isnan.(values(percentchange(cl; padding=true))[1])
            @test values(percentchange(cl; padding=true))[2] ≈ (102.5 - 111.94) / 111.94 atol =
                0.01
            @test values(percentchange(ohlc; padding=true))[2, :] ≈
                (values(ohlc)[2, :] - values(ohlc)[1, :]) ./ values(ohlc)[1, :]
        end

        @testset "log return value" begin
            @test values(percentchange(cl, :log)) ==
                values(percentchange(cl, :log; padding=false))
            @test values(percentchange(cl, :log))[1] ≈ log(102.5) - log(111.94) atol = 0.01
            @test values(percentchange(ohlc, :log))[1, :] ≈
                log.(values(ohlc)[2, :]) .- log.(values(ohlc)[1, :]) atol = 0.01
            @test isnan.(values(percentchange(cl, :log; padding=true))[1])
            @test values(percentchange(cl, :log; padding=true))[2] ≈
                log(102.5) - log(111.94)
            @test values(percentchange(ohlc, :log; padding=true))[2, :] ≈
                log.(values(ohlc)[2, :]) .- log.(values(ohlc)[1, :])
        end
    ...
    @testset "adding/removing missing rows works" begin
        ...
        @testset "dropnan works" begin
            nohlc = TimeArray(
                timestamp(ohlc), copy(values(ohlc)), colnames(ohlc), meta(ohlc)
            )
