...
facts("type constructors enforce invariants") do
    ...
    context("metadata defaults to Nothing") do
        @fact cl.metadata => Nothing
    end
  
    context("metadata field can be assigned") do
        clm = TimeArray(cl.timestamp, cl.values, cl.colnames, 2)
        @fact typeof(clm.metadata) => Int
        @fact clm.metadata         => 2
    end
end
