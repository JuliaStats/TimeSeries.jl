...
@testset "broadcast" begin
    @testset "base element-wise operators on TimeArray values" begin
        @testset "only values on intersecting Dates computed" begin
            ...
            let
                ta = cl[1:4] .+ op[4:7]
                @test values(ta)[1] ≈ 201.12 atol = 0.01
                @test length(ta) == 1
            end
        end
        ...
        @testset "dot operation between TimeArray values and Int/Float and viceversa" begin
            @test values(cl .- 100)[1] ≈ 11.94 atol = 0.01
            @test values(cl .+ 100)[1] ≈ 211.94 atol = 0.01
            @test values(cl .* 100)[1] ≈ 11194 atol = 1
            @test values(cl ./ 100)[1] ≈ 1.1194 atol = 0.0001
            @test values(cl .^ 2)[1] ≈ 12530.5636 atol = 0.0001
            @test values(cl .% 2)[1] == values(cl)[1] % 2
            @test values(100 .- cl)[1] ≈ -11.94 atol = 0.01
            @test values(100 .+ cl)[1] ≈ 211.94 atol = 0.01
            @test values(100 .* cl)[1] ≈ 11194 atol = 0.01
        ...
        @testset "mathematical operations between two same-column-count TimeArrays" begin
            @test values(cl .+ op)[1] ≈ 216.82 atol = 0.01
            @test values(cl .- op)[1] ≈ 7.06 atol = 0.01
